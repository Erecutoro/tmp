...
def _create_sec_websocket_key():
    uid = uuid.uuid4()
    return decode(base64.encodebytes(uid.bytes).strip())
...
class WebSocket(object):
    ...
    def _validate_header(self, headers, key):
        ...
        hashed = decode(
            base64.encodebytes(hashlib.sha1(encode(value)).digest()).strip().lower()
        )
...
def main(domain, port, user, garbage_path, secure, verbose):
    ...
    try:
        enableTrace(verbose)
        connection_url = "wss://%s:%s/%s" % (domain, port, __path__)
        display_status(
